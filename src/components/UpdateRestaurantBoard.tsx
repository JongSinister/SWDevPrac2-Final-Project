...
export default function UpdateRestaurantBoard({ rid }: { rid: string }) {
  ...
  // Fetch the restaurant details on load and set form fields
  useEffect(() => {
    async function fetchRestaurant() {
      const restaurant = await getRestaurant(rid);
      setName(restaurant.data.name);
      setAddress(restaurant.data.address);
      setFoodType(restaurant.data.foodtype);
      setProvince(restaurant.data.province);
      setPostalcode(restaurant.data.postalcode);
      setTel(restaurant.data.tel);
      setPicture(restaurant.data.picture);
    }
    fetchRestaurant();
  }, [rid]);
  ...
  const handleUpdateRestaurant = async () => {
    ...
    await editRestaurant(rid, restaurant);
